...
def coast_until(tf, sc, launch_ut, phase, predicate, timeout=3600):
    ...
    while not predicate():
        vessel = refresh_vessel(sc)
        write_telemetry(tf, sc, launch_ut, phase)
        if vessel.orbit.eccentricity >= 1.0 or vessel.orbit.apoapsis_altitude < 0 or vessel.orbit.apoapsis_altitude > 1_000_000:
            raise RuntimeError(f'Abort coast: runaway/hyperbolic orbit in {phase}')
        if sc.ut - t0 > timeout:
...
def set_pitch_for_ascent(ap, altitude):
    if altitude < 1000:
        ...
        frac = (altitude - 1000.0) / 34000.0
        pitch = 90.0 - 89.0 * frac
    else:
        pitch = 1.0
...
def active_srb_engines(vessel):
    try:
        return [e for e in vessel.parts.engines if e.active and 'Thumper' in e.part.title]
    except Exception:
        return []
...
def active_non_srb_engines(vessel):
    try:
        return [e for e in vessel.parts.engines if e.active and 'Thumper' not in e.part.title]
    except Exception:
        return []
...
def stage_to_upper(sc, ef, launch_ut):
    ...
    vessel.control.activate_next_stage()
    time.sleep(1.5)
    write_event(ef, sc, launch_ut, f"STAGE_{refresh_vessel(sc).control.current_stage}", 'core_sep_upper_ignite')
...
launch_ut_main = None
try:
    conn = krpc.connect(name='flight_attempt_5')
    ...
    with open(TEL_FILE, 'w') as tf, open(BURNS_FILE, 'w') as bf, open(EVENTS_FILE, 'w') as ef:
        ...
        while True:
            ...
            vessel.control.throttle = 0.65 if q > 18000 else 1.0
            ...
            orbit = refresh_vessel(sc).orbit
            if orbit.apoapsis_altitude >= target_ap:
                refresh_vessel(sc).control.throttle = 0.0
                write_event(ef, sc, launch_ut_main, 'APOAPSIS_COAST_START', f'AP={orbit.apoapsis_altitude:.0f}')
                break

            if orbit.apoapsis_altitude > 120000 or orbit.eccentricity >= 1.0:
                refresh_vessel(sc).control.throttle = 0.0
                raise RuntimeError(f'Ascent runaway: AP={orbit.apoapsis_altitude:.0f} ECC={orbit.eccentricity:.4f}')

            if alt > 70000 and orbit.apoapsis_altitude < 70000:
                raise RuntimeError('Left atmosphere before setting apoapsis target')

            write_telemetry(tf, sc, launch_ut_main, 'ASCENT')
            time.sleep(0.1)

        coast_until(tf, sc, launch_ut_main, 'TO_AP_FOR_CIRC', lambda: refresh_vessel(sc).orbit.time_to_apoapsis < 20, timeout=2000)
        vessel = refresh_vessel(sc)
        ...
        while refresh_vessel(sc).orbit.body.name != 'Mun':
            ...
            if vessel.orbit.eccentricity >= 1.0 and vessel.orbit.body.name == 'Kerbin' and vessel.orbit.next_orbit is None:
                raise RuntimeError('Escaped Kerbin without Mun encounter')
            try:
