...
def execute_node(conn, vessel, sc, tf, bf, ef, launch_ut, node, phase_name):
    ...
    try:
        ...
        write_event(ef, vessel, sc, launch_ut, f"BURN_END:{phase_name}")
        try:
            node.remove()
...
launch_ut_main = None
try:
    conn = krpc.connect(name='flight_attempt_2')
    sc = conn.space_center
    vessel = sc.active_vessel

    with open(TEL_FILE, 'w') as tf, open(BURNS_FILE, 'w') as bf, open(EVENTS_FILE, 'w') as ef:
        launch_ut_main = sc.ut
        write_event(ef, vessel, sc, launch_ut_main, 'SCRIPT_START')
        write_telemetry(tf, vessel, sc, launch_ut_main, 'PRELAUNCH')
        ...
        vessel.control.activate_next_stage()
        write_event(ef, vessel, sc, launch_ut_main, 'LAUNCH')
        write_event(ef, vessel, sc, launch_ut_main, f'STAGE_{vessel.control.current_stage}', 'liftoff')
        ...
        while True:
            ...
            if alt > 1000 and not gt_started:
                gt_started = True
                write_event(ef, vessel, sc, launch_ut_main, 'GRAVITY_TURN_START')
            ...
            if not srb_sep_done:
                ...
                if srbs and all((not e.has_fuel) for e in srbs):
                    vessel.control.activate_next_stage()
                    srb_sep_done = True
                    write_event(ef, vessel, sc, launch_ut_main, f'STAGE_{vessel.control.current_stage}', 'srb_separation')
                    time.sleep(1.0)

            if vessel.orbit.apoapsis_altitude >= target_ap:
                vessel.control.throttle = 0.0
                write_event(ef, vessel, sc, launch_ut_main, 'APOAPSIS_COAST_START', f'AP={vessel.orbit.apoapsis_altitude:.0f}')
                break

            if alt > 70000 and vessel.orbit.apoapsis_altitude < 80000:
                raise RuntimeError('Left atmosphere before setting apoapsis target')

            stage_if_no_fuel(vessel, ef, sc, launch_ut_main, 'ascent_auto_stage')
            write_telemetry(tf, vessel, sc, launch_ut_main, 'ASCENT')
            time.sleep(0.1)

        coast_until(tf, vessel, sc, launch_ut_main, 'TO_AP_FOR_CIRC', lambda: vessel.orbit.time_to_apoapsis < 25, timeout=4000)
        ...
        write_event(ef, vessel, sc, launch_ut_main, 'NODE_CREATED:CIRCULARIZE', f'dv={dv_circ:.2f}')
        execute_node(conn, vessel, sc, tf, bf, ef, launch_ut_main, node, 'CIRCULARIZE')
        ...
        write_event(ef, vessel, sc, launch_ut_main, 'LKO_CONFIRMED', f'AP={vessel.orbit.apoapsis_altitude:.0f} PE={vessel.orbit.periapsis_altitude:.0f}')

        mun = sc.bodies['Mun']
        node, score = plan_tmi_node(vessel, sc, mun, ef, launch_ut_main)
        execute_node(conn, vessel, sc, tf, bf, ef, launch_ut_main, node, 'TMI')

        try:
            if vessel.orbit.next_orbit and vessel.orbit.next_orbit.body.name == 'Mun':
                write_event(ef, vessel, sc, launch_ut_main, 'ENCOUNTER_CONFIRMED', f'pe={vessel.orbit.next_orbit.periapsis_altitude:.0f}')
        except Exception:
            pass

        t0 = sc.ut
        while vessel.orbit.body.name != 'Mun':
            write_telemetry(tf, vessel, sc, launch_ut_main, 'TMI_COAST')
            try:
        ...
        write_event(ef, vessel, sc, launch_ut_main, 'SOI_CHANGE:Mun')
        moi_node = plan_moi_node(vessel, sc, ef, launch_ut_main, target_pe_alt=30000.0)
        execute_node(conn, vessel, sc, tf, bf, ef, launch_ut_main, moi_node, 'MOI')
        ...
        if vessel.orbit.periapsis_altitude < 10000 or vessel.orbit.apoapsis_altitude > 500000:
            trim_node = plan_trim_node(vessel, sc, ef, launch_ut_main, target_ap_alt=min(max(vessel.orbit.apoapsis_altitude, 30000.0), 120000.0))
            execute_node(conn, vessel, sc, tf, bf, ef, launch_ut_main, trim_node, 'TRIM')
        ...
        confirm_mun_orbit(tf, vessel, sc, ef, launch_ut_main)
        write_telemetry(tf, vessel, sc, launch_ut_main, 'MISSION_COMPLETE')
    ...
    try:
        if conn is not None:
            sc = conn.space_center
            vessel = sc.active_vessel
            base_ut = launch_ut_main if launch_ut_main is not None else sc.ut - getattr(vessel, 'met', 0.0)
            with open(EVENTS_FILE, 'a') as ef:
                write_event(ef, vessel, sc, base_ut, 'SCRIPT_EXCEPTION', repr(exc))
                safe_write(ef, traceback.format_exc() + "\n")
            with open(TEL_FILE, 'a') as tf:
                write_telemetry(tf, vessel, sc, base_ut, 'EXCEPTION')
    except Exception:
        pass
    raise
