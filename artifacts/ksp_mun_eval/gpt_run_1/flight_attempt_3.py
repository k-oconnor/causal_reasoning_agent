...
def set_pitch_for_ascent(vessel, ap, altitude):
    if altitude < 1000:
        pitch = 90.0
    elif altitude < 45000:
...
def stage_if_no_fuel(sc, ef, launch_ut, label):
    vessel = refresh_vessel(sc)
    try:
    ...
    if active and all((not e.has_fuel) for e in active):
        ...
        time.sleep(1.0)
        write_event(ef, sc, launch_ut, f"STAGE_{refresh_vessel(sc).control.current_stage}", label)
        write_event(ef, sc, launch_ut, "VESSEL_REFRESH", label)
        return True
    return False
...
def execute_node(conn, sc, tf, bf, ef, launch_ut, node, phase_name):
    ...
    try:
        while True:
            ...
            if stage_if_no_fuel(sc, ef, launch_ut, f"during_{phase_name}"):
                vessel, ap = orient_to_node(sc, node)
            time.sleep(0.05)
    finally:
        vessel = refresh_vessel(sc)
...
launch_ut_main = None
try:
    conn = krpc.connect(name='flight_attempt_3')
    ...
    with open(TEL_FILE, 'w') as tf, open(BURNS_FILE, 'w') as bf, open(EVENTS_FILE, 'w') as ef:
        ...
        while True:
            ...
            set_pitch_for_ascent(vessel, ap, alt)
            ...
            if not srb_sep_done:
                ...
                if srbs and all((not e.has_fuel) for e in srbs):
                    ...
                    srb_sep_done = True
                    time.sleep(1.0)
                    write_event(ef, sc, launch_ut_main, f'STAGE_{refresh_vessel(sc).control.current_stage}', 'srb_separation')
                    write_event(ef, sc, launch_ut_main, 'VESSEL_REFRESH', 'after_srb_sep')

            if vessel.orbit.apoapsis_altitude >= target_ap:
                vessel.control.throttle = 0.0
                write_event(ef, sc, launch_ut_main, 'APOAPSIS_COAST_START', f'AP={vessel.orbit.apoapsis_altitude:.0f}')
                break

            if alt > 70000 and vessel.orbit.apoapsis_altitude < 80000:
                raise RuntimeError('Left atmosphere before setting apoapsis target')

            stage_if_no_fuel(sc, ef, launch_ut_main, 'ascent_auto_stage')
        ...
        mun = sc.bodies['Mun']
        write_event(ef, sc, launch_ut_main, 'VESSEL_REFRESH', 'before_tmi_node')
        node, score = plan_tmi_node(sc, mun, ef, launch_ut_main)
        ...
        write_event(ef, sc, launch_ut_main, 'VESSEL_REFRESH', 'before_moi_node')
