"""
KSP Mun Mission — Attempt 4
3-stage rocket: Skipper (stage 1) → Terrier (stage 2) → Spark (stage 3)
Target: Stable Mun orbit (PE >= 10 km, AP <= 500 km)
Key fix from A3: Eliminate coast phase. Circularize immediately after first stage.
Also: set throttle to 0 explicitly on every loop iteration as a guard.
"""
...
try:
    ...
    # ============================================================
    # PHASE 1: LAUNCH AND ASCENT — Burn first stage to depletion
    # ============================================================
    ...
    # Ascend — burn the first stage (Skipper) until it runs out
    ...
    while not first_stage_depleted:
        ...
        # Check if first stage is depleted
    ...
    # ============================================================
    # PHASE 2: IMMEDIATE CIRCULARIZATION (no coast)
    # ============================================================
    print("=== PHASE 2: IMMEDIATE CIRCULARIZATION ===")

    # Immediately set throttle to 0 and wait for staging to settle
    vessel.control.throttle = 0.0
    time.sleep(1.0)

    # Make sure we're pointing prograde
    ap.reference_frame = vessel.surface_reference_frame
    ap.target_direction = (0.0, 1.0, 0.0)  # prograde in surface frame
    ap.engage()
    time.sleep(1.0)

    # Compute how much dV we need to circularize NOW
    # We want PE >= 75 km. Burn prograde at current position.
    mu_k = vessel.orbit.body.gravitational_parameter
    r_now = vessel.orbit.radius  # current distance from body centre
    a_now = vessel.orbit.semi_major_axis
    v_now = vessel.orbit.speed

    # Target: circular orbit at current altitude
    target_pe = 75000  # m
    target_radius = vessel.orbit.body.equatorial_radius + target_pe

    # We need to raise periapsis to target_pe.
    # Using vis-viva: we want the new orbit's periapsis = target_radius
    # At current position r_now, we need speed v_circ such that
    # the new orbit has periapsis = target_radius.
    # For an orbit with periapsis = target_radius and current position r_now:
    # a_new = (r_now + target_radius) / 2
    # v_needed = sqrt(mu * (2/r_now - 1/a_new))
    a_new = (r_now + target_radius) / 2.0
    v_needed = math.sqrt(mu_k * (2.0 / r_now - 1.0 / a_new))
    circ_dv = v_needed - v_now

    write_event("CIRC_DV_COMPUTED", f"dV={circ_dv:.1f}m/s r_now={r_now:.0f}m a_new={a_new:.0f}m")

    if circ_dv < 0 or circ_dv > 1000:
        write_event("CIRC_DV_SANITY_FAIL", f"dV={circ_dv:.1f}")
        raise RuntimeError(f"Circularization dV out of range: {circ_dv:.1f}")

    # Create and execute circularization node at current time
    ...
    for i in range(600):
        ...
        # THROTTLE GUARD: ensure throttle stays at 0 during coast
        vessel.control.throttle = 0.0
