"""
KSP Mun Mission — Attempt 3
3-stage rocket: Skipper (stage 1) → Terrier (stage 2) → Spark (stage 3)
Target: Stable Mun orbit (PE >= 10 km, AP <= 500 km)
Key fix from A2: Burn first stage to depletion instead of cutting at AP target
"""
...
try:
    ...
    # Ascend — burn the first stage (Skipper) until it runs out of fuel
    ...
    while not first_stage_depleted:
        ...
        if dyn_q > 20000:
            ...
            # Restore throttle once past max Q
            vessel.control.throttle = 1.0

        # Track max Q
        if not max_q_reached and dyn_q > 15000:
            max_q_reached = True
            write_event("MAX_Q", f"Q={dyn_q:.0f}Pa ALT={alt:.0f}m")

        # Check if first stage is depleted
        if stage_if_depleted():
            # Staging happened — check if we're still on stage 1
            if vessel.control.current_stage <= 3:  # Stage 1 was stage 5, now should be 4 or lower
                first_stage_depleted = True
                write_event("FIRST_STAGE_DEPLETED",
                            f"ALT={alt:.0f}m SPD={orbit.speed:.1f}m/s AP={ap_alt:.0f}m")
                break

        time.sleep(0.1)

    # ============================================================
    # Coast to apoapsis with upper stage (Terrier or Spark)
    # ============================================================
    print("=== COASTING TO APOAPSIS ===")
    write_event("APOAPSIS_COAST_START")

    # Keep throttle at 0 during coast
    vessel.control.throttle = 0.0
